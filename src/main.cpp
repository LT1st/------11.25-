...
int flg = 0,wait = 0,wait1 = 0,readColor = 0,Cnt = 0;
...
int main()
{
    ...
    while (cvGetWindowHandle(depth_win)&&cvGetWindowHandle(color_win)) // Application still alive?
    {
        ...
        roi_region_col =  imgCols/2;
        roi_region_col1 = imgCols;
        ...
        roi_region_col1 = color_image.cols;
        roi_region_col  = color_image.cols/2;
        //CvPoint 
        ...
        //找到的圆的数量：
        cout << "找到的圆的数量:" << circles.size() << endl;
        ...
        if(circles.size() != 0)
        {
            ...
            //cout<<"\n\n\n错误："<<circles[0][0]<< "\t"<<range_ifcolor.width/2 
            //<< "\t"<< circles[0][1]<<endl;
            Rect RectRange_ifcolor(cvRound(circles[0][0])-range_ifcolor.width/2,cvRound(circles[0][1])-range_ifcolor.height/2,range_ifcolor.width,range_ifcolor.height);
            //遍历该范围
        ...
        /*  Vec3f effective_distance=midH/effective_pixel;
            ...
            cout << "HSV:" << mid_color_image.at<Vec3f>(cvRound(circles[0][0]),cvRound(circles[0][1])) << endl;
            ...
            // cout << "读到的颜色:" << readColor << endl;
            // cout << "分割出来的是:" << g_ball_color <<endl;
            // if(readColor == g_ball_color)
            // {
            //     cout << "匹配到:" << readColor << endl; 
            //     flg = 1;
            // }
            // if(flg == 1)
            // {
            ...
            centerLocation_x = cvRound(circles[0][0]) + roi_region_col;
            centerLocation_y = cvRound(circles[0][1]) + roi_region_row;
            
            //输出当前roi
            //cout << roi_region_row << "\troi_region_   " << roi_region_col << "\t+  cvRound   " <<  cvRound(circles[0][0]) << "\t"
